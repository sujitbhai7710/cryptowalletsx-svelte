<script lang="ts">
  ...
  import type { JumperStats, JumperChainActivity, JumperProtocolUsage } from '$lib/types';
  ...
  let showAllProtocols = $state(false);
  ...
  let displayedProtocols = $derived(showAllProtocols && stats ? stats.protocolUsage : (stats ? stats.protocolUsage.slice(0, 15) : []));
  let displayedTxs = $derived(showAllTxs && stats ? stats.transfers : (stats ? stats.transfers.slice(0, 25) : []));
...
<div class="min-h-screen flex flex-col">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
      ...
      <div class="pb-2 flex flex-wrap items-center gap-1.5 sm:gap-2 wallet-bar">
        <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-secondary/80">
          <ArrowLeftRight class="w-3.5 h-3.5 text-indigo-500 shrink-0" />
          ...
          <button class="p-0.5 hover:bg-secondary rounded" onclick={handleCopy}>
            {#if copied}<Check class="w-3 h-3 text-indigo-500" />{:else}<Copy class="w-3 h-3" />{/if}
          </button>
        </div>
        <div class="flex items-center gap-0.5 sm:gap-1">
          <a href="https://jumper.exchange" target="_blank" rel="noopener noreferrer">
            <Button variant="ghost" size="sm" class="h-7 text-xs gap-1">
              <ExternalLink class="w-3 h-3" />
              <span class="hidden sm:inline">Jumper</span>
            </Button>
          </a>
    ...
    {#if mobileMenuOpen}
      <div class="md:hidden border-t border-border/40 bg-background/95 backdrop-blur-xl">
        <nav class="max-w-7xl mx-auto px-4 py-3 space-y-1">
          ...
          <a href="/checker" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-secondary/60"><ArrowLeftRight class="w-4 h-4" /> All Checkers</a>
        </nav>
      </div>
    {/if}
  </header>
    ...
    <div class="max-w-7xl mx-auto px-2 sm:px-6">
      <div class="w-full flex h-auto p-1 bg-transparent gap-0.5 overflow-x-auto tab-scroll-container" style="-webkit-overflow-scrolling: touch; scrollbar-width: none;">
        {#each SECTIONS as section}
          <button
            class="text-xs sm:text-sm px-3 sm:px-4 py-2 whitespace-nowrap rounded-lg transition-all shrink-0 {activeSection === section.id ? 'bg-indigo-500/15 text-indigo-600 dark:text-indigo-400 font-semibold' : 'text-muted-foreground hover:text-foreground'}"
  ...
  <main class="flex-1 max-w-7xl mx-auto px-4 sm:px-6 py-6 w-full">
    {#if isLoading}
      <WalletLoading chainName="Jumper" chainColor="purple" message="Analyzing Jumper bridge & swap data" />
    {:else if loadError}
      <div class="text-center py-16">
        <div class="text-4xl mb-4">⚠️</div>
        <h2 class="text-xl font-bold mb-2">Failed to Load Data</h2>
        <p class="text-muted-foreground mb-4">{loadError}</p>
        <Button onclick={onRetry} class="bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
    ...
    {:else if stats}
      {#if stats.totalTransfers === 0}
        <div class="text-center py-16">
          ...
          <h2 class="text-xl font-bold mb-2">No Jumper Activity Found</h2>
          <p class="text-muted-foreground">This wallet has no Jumper bridge or swap transactions.</p>
        </div>
      {:else if activeSection === 'overview'}
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            ...
            <Card class="bg-card/60 border-border/40">
              <div class="p-4 text-center">
                ...
                <p class="font-bold text-xl text-indigo-500">{stats.daysActive}</p>
              </div>
            </Card>
            <Card class="bg-card/60 border-border/40">
              <div class="p-4 text-center">
                <p class="text-xs text-muted-foreground mb-1">Weeks</p>
                <p class="font-bold text-xl text-purple-500">{stats.weeksActive}</p>
              </div>
            </Card>
            <Card class="bg-card/60 border-border/40">
              <div class="p-4 text-center">
                <p class="text-xs text-muted-foreground mb-1">Months</p>
                <p class="font-bold text-xl text-indigo-400">{stats.monthsActive}</p>
              </div>
            </Card>
          </div>
        </div>
        ...
        <div class="mb-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Card class="bg-gradient-to-br from-indigo-500/10 to-indigo-600/5 border-indigo-500/20">
              <div class="p-4">
                <p class="text-xs text-muted-foreground mb-1">Total Bridges</p>
                <p class="font-bold text-2xl">{stats.totalTransfers}</p>
                <p class="text-[10px] text-muted-foreground mt-1">+{stats.sevenDayTransfers} (7d)</p>
              </div>
            </Card>
            <Card class="bg-gradient-to-br from-purple-500/10 to-purple-600/5 border-purple-500/20">
              <div class="p-4">
                ...
                <p class="text-[10px] text-muted-foreground mt-1 {stats.sevenDayVolumeChange >= 0 ? 'text-emerald-500' : 'text-red-500'}">
                  {stats.sevenDayVolumeChange >= 0 ? '+' : ''}{stats.sevenDayVolumeChange.toFixed(1)}% (7d)
                </p>
              </div>
            </Card>
            <Card class="bg-gradient-to-br from-violet-500/10 to-violet-600/5 border-violet-500/20">
              <div class="p-4">
                ...
                <p class="font-bold text-2xl">{formatVolume(stats.totalFees)}</p>
                <p class="text-[10px] text-muted-foreground mt-1">Gas + bridge fees</p>
              </div>
            </Card>
            <Card class="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border-blue-500/20">
              <div class="p-4">
                <p class="text-xs text-muted-foreground mb-1">Chains Used</p>
        ...
        <div class="mb-6">
          <h3 class="text-lg font-bold mb-3 flex items-center gap-2">
            <ArrowUpDown class="w-4 h-4 text-indigo-500" />
            Bridge vs Swap
          </h3>
              ...
              <div class="p-5">
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    ...
                    <p class="font-bold text-lg">{stats.bridgeTransfers}</p>
                  </div>
                  <div>
            ...
            <Card class="bg-card/60 border-border/40 overflow-hidden">
              <div class="p-5">
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    <p class="text-xs text-muted-foreground">Transfers</p>
                    <p class="font-bold text-lg">{stats.swapTransfers}</p>
                  </div>
                  <div>
                    <p class="text-xs text-muted-foreground">Volume</p>
                ...
                <div class="h-2 rounded-full bg-secondary overflow-hidden">
                  <div
                    class="h-full rounded-full bg-gradient-to-r from-purple-500 to-violet-500 transition-all duration-500"
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            <Card class="bg-card/60 border-border/40">
              <div class="p-4 text-center">
                ...
                <p class="font-bold text-xl text-indigo-500">{stats.sourceChains}</p>
              </div>
            </Card>
            <Card class="bg-card/60 border-border/40">
              <div class="p-4 text-center">
                <p class="text-xs text-muted-foreground mb-1">Dest. Chains</p>
                <p class="font-bold text-xl text-purple-500">{stats.destChains}</p>
              </div>
            </Card>
            <Card class="bg-card/60 border-border/40">
              <div class="p-4 text-center">
              ...
              <div class="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div class="relative w-40 h-40 shrink-0">
                  <svg viewBox="0 0 100 100" class="w-full h-full -rotate-90">
                    ...
                    {/if}
                    <defs>
                      <linearGradient id="jBridgeGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop offset="0%" style="stop-color:#6366f1" />
                        <stop offset="100%" style="stop-color:#8b5cf6" />
                      </linearGradient>
                      <linearGradient id="jSwapGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                  ...
                  <div class="absolute inset-0 flex items-center justify-center">
                    <div class="text-center">
                      <p class="font-bold text-lg">{stats.totalTransfers}</p>
                ...
                <div class="flex-1 space-y-3 w-full">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 flex items-center justify-between">
                      <span class="text-sm">Bridge Transfers</span>
                      <span class="font-bold text-sm">{stats.bridgeTransfers} ({stats.totalTransfers > 0 ? (stats.bridgeTransfers / stats.totalTransfers * 100).toFixed(1) : 0}%)</span>
                    </div>
                  </div>
                  <div class="flex items-center gap-3">
                    <div class="w-3 h-3 rounded-full bg-purple-500 shrink-0"></div>
                    <div class="flex-1 flex items-center justify-between">
                      <span class="text-sm">Swap Transfers</span>
                      <span class="font-bold text-sm">{stats.swapTransfers} ({stats.totalTransfers > 0 ? (stats.swapTransfers / stats.totalTransfers * 100).toFixed(1) : 0}%)</span>
                    </div>
                  </div>
              ...
              {#each topChainsForChart as chain}
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1 h-5 rounded-full bg-secondary/60 overflow-hidden relative">
                    <div class="h-full rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 transition-all duration-500" style="width: {pct}%"></div>
      ...
      {:else if activeSection === 'protocols'}
        ...
        <div class="mb-4">
          ...
          <p class="text-sm text-muted-foreground">{stats.protocolUsage.length} Protocols &bull; Bridges & Exchanges</p>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <Card class="bg-gradient-to-br from-indigo-500/10 to-indigo-600/5 border-indigo-500/20">
            <div class="p-4 text-center">
              <p class="text-xs text-muted-foreground mb-1">Bridges Used</p>
              <p class="font-bold text-xl text-indigo-500">{stats.protocolUsage.filter(p => p.type === 'Bridge').length}</p>
            </div>
          </Card>
          <Card class="bg-gradient-to-br from-purple-500/10 to-purple-600/5 border-purple-500/20">
            <div class="p-4 text-center">
              <p class="text-xs text-muted-foreground mb-1">Exchanges Used</p>
              <p class="font-bold text-xl text-purple-500">{stats.protocolUsage.filter(p => p.type === 'Exchange').length}</p>
            </div>
          </Card>
          <Card class="bg-gradient-to-br from-violet-500/10 to-violet-600/5 border-violet-500/20">
            <div class="p-4 text-center">
              <p class="text-xs text-muted-foreground mb-1">Bridge Txs</p>
              <p class="font-bold text-xl">{stats.bridgeTransfers}</p>
            </div>
          </Card>
          <Card class="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border-blue-500/20">
            <div class="p-4 text-center">
              <p class="text-xs text-muted-foreground mb-1">Exchange Txs</p>
              <p class="font-bold text-xl">{stats.swapTransfers}</p>
            </div>
          </Card>
        </div>
        ...
        <Card class="bg-card/60 border-border/40 overflow-hidden">
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-border/40 bg-muted/30">
                  <th class="text-left px-4 py-3 font-medium text-muted-foreground">Protocol</th>
                  <th class="text-center px-3 py-3 font-medium text-muted-foreground">Type</th>
              ...
              <tbody>
                {#each displayedProtocols as protocol}
                  <tr class="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td class="text-center px-3 py-3">
                      <Badge class="text-[10px] {protocol.type === 'Bridge' ? 'bg-indigo-500/15 text-indigo-500 border-indigo-500/30' : 'bg-purple-500/15 text-purple-500 border-purple-500/30'}">
                        {protocol.type}
                      </Badge>
                    </td>
                    <td class="text-right px-3 py-3 font-mono text-xs">{protocol.transactions}</td>
                    <td class="text-right px-3 py-3 font-mono text-xs">{formatVolume(protocol.volume)}</td>
                  </tr>
                {/each}
              </tbody>
            </table>
          </div>
          {#if stats.protocolUsage.length > 15}
            <div class="p-3 text-center border-t border-border/40">
              <button
                class="text-sm text-indigo-500 hover:text-indigo-400 font-medium"
                onclick={() => showAllProtocols = !showAllProtocols}
              >
                {showAllProtocols ? 'Show Less' : `Show All Protocols (${stats.protocolUsage.length - 15} more)`}
              </button>
            </div>
          {/if}
        </Card>
      ...
      {:else if activeSection === 'history'}
        ...
        <div class="mb-4">
          ...
          <p class="text-sm text-muted-foreground">{stats.totalTransfers} Transactions &bull; Recent activity</p>
        </div>
        ...
        <Card class="bg-card/60 border-border/40 overflow-hidden">
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-border/40 bg-muted/30">
                  ...
                  <th class="text-left px-3 py-3 font-medium text-muted-foreground hidden md:table-cell">From Chain</th>
                  <th class="text-left px-3 py-3 font-medium text-muted-foreground hidden md:table-cell">To Chain</th>
                  <th class="text-right px-3 py-3 font-medium text-muted-foreground">Send</th>
                  <th class="text-right px-3 py-3 font-medium text-muted-foreground hidden sm:table-cell">Receive</th>
                  <th class="text-center px-3 py-3 font-medium text-muted-foreground">Status</th>
                </tr>
              </thead>
              <tbody>
                {#each displayedTxs as tx}
                  <tr class="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td class="px-2 py-2.5 sm:px-3 sm:py-3">
                      <Badge class="text-[9px] sm:text-[10px] {tx.type === 'Bridge' ? 'bg-indigo-500/15 text-indigo-500 border-indigo-500/30' : 'bg-purple-500/15 text-purple-500 border-purple-500/30'}">
                        {tx.type}
                      </Badge>
                    </td>
                    ...
                    <td class="text-right px-2 py-2.5 sm:px-3 sm:py-3">
                      <div class="text-xs font-medium">${tx.fromAmountUSD}</div>
                      <div class="text-[10px] text-muted-foreground">{tx.sourceToken}</div>
                    </td>
                    <td class="text-right px-2 py-2.5 sm:px-3 sm:py-3 hidden sm:table-cell">
                      <div class="text-xs font-medium">${tx.toAmountUSD}</div>
                      <div class="text-[10px] text-muted-foreground">{tx.destToken}</div>
                    </td>
                    <td class="text-center px-3 py-3">
                      <Badge class="text-[10px] {tx.status === 'Success' ? 'bg-emerald-500/15 text-emerald-500 border-emerald-500/30' : tx.status === 'Failed' ? 'bg-red-500/15 text-red-500 border-red-500/30' : 'bg-amber-500/15 text-amber-500 border-amber-500/30'}">
                        {tx.status}
                      </Badge>
                    </td>
                  </tr>
          ...
          {#if stats.transfers.length > 25}
            <div class="p-3 text-center border-t border-border/40">
              <button
                class="text-sm text-indigo-500 hover:text-indigo-400 font-medium"
                onclick={() => showAllTxs = !showAllTxs}
              >
                {showAllTxs ? 'Show Less' : `Show All Transactions (${stats.transfers.length - 25} more)`}
              </button>
            </div>
          {/if}
        </Card>
