<script lang="ts">
  ...
  import { RELAY_CONFIG } from '$lib/utils/constants';
...
<div class="min-h-screen flex flex-col">
  ...
  <header class="sticky top-0 z-50 border-b border-border/40 bg-background/80 backdrop-blur-xl">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
      <div class="flex items-center gap-3 min-w-0">
        ...
        <span class="font-bold text-lg gradient-text truncate">Relay Bridge Analytics</span>
  ...
  <main class="flex-1">
    <section class="relative overflow-hidden">
      ...
      <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-gradient-to-r from-orange-500/8 to-amber-500/8 rounded-full blur-3xl"></div>

      <div class="relative max-w-4xl mx-auto px-4 sm:px-6 pt-16 sm:pt-24 pb-12 text-center">
        <Badge class="mb-6 bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20 hover:bg-orange-500/15">
        ...
        <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight mb-4">
          <span class="gradient-text">Relay</span>
          <br />
          <span class="text-foreground">Bridge Analytics</span>
        </h1>

        <p class="text-muted-foreground text-lg sm:text-xl max-w-2xl mx-auto mb-10">
          Comprehensive cross-chain analytics for Relay bridge and swap activity.
          Track your multi-chain transactions, volumes, and network coverage.
        </p>

        <!-- Search Input -->
        <div class="max-w-xl mx-auto">
          <div class="flex flex-col sm:flex-row gap-2 p-2 bg-card border border-border/50 rounded-2xl shadow-lg shadow-orange-500/5 backdrop-blur-xl">
            <Input
              value={inputAddress}
            ...
            <Button
              onclick={handleScan}
              class="bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white px-6 rounded-xl shadow-md shadow-orange-500/25"
            >
    ...
    <section class="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        {#each [
          { icon: Globe, label: 'Multi-Chain', value: '82+ Networks', color: 'text-orange-500' },
          { icon: ArrowLeftRight, label: 'Supported', value: 'Bridge & Swap', color: 'text-amber-500' },
          { icon: Activity, label: 'Type', value: 'Cross-Chain Dapp', color: 'text-orange-500' },
          { icon: Wallet, label: 'Protocol', value: 'Relay.link', color: 'text-amber-500' },
        ] as stat}
    ...
    <section class="max-w-6xl mx-auto px-4 sm:px-6 py-8 pb-16">
      <h2 class="text-2xl font-bold mb-6 text-center">
        <ArrowLeftRight class="w-5 h-5 inline text-orange-500" />
        What You Can Analyze
      </h2>
      <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {#each [
          { title: 'Bridge Activity', desc: 'Track all cross-chain bridge transfers across 82+ networks with volume analysis', emoji: '🌉' },
          { title: 'Swap Analytics', desc: 'Monitor same-chain and cross-chain swap activity with token pair breakdown', emoji: '🔄' },
          { title: 'Chain Coverage', desc: 'See which networks you have used as source and destination chains', emoji: '🌐' },
          { title: 'Volume Tracking', desc: 'USD-denominated volume analysis with per-chain and per-token breakdowns', emoji: '📊' },
          { title: 'Success Rate', desc: 'Transaction success/failure ratio across all your Relay interactions', emoji: '✅' },
          { title: 'Token Usage', desc: 'Detailed token-level analysis showing which tokens you bridge and swap most', emoji: '🪙' },
        ] as feat}
    ...
    <!-- SEO Content -->
    <ToolSEOContent config={RELAY_CONFIG} />
  </main>
  ...
  <footer class="border-t border-border/40 bg-card/30 mt-auto">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
      <div class="flex items-center gap-2">
        <div class="w-6 h-6 rounded-md bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center">
          <Zap class="w-3 h-3 text-white" />
        </div>
        <span class="text-sm font-medium gradient-text">Relay Bridge Analytics</span>
      </div>
